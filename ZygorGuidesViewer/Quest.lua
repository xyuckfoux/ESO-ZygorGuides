...
function Quests:RemoveQuest(journalIndex)
	if type(journalIndex)=="string" then journalIndex=self:FindQuest(journalIndex) end
...
function Quests:GetQuest(journalIndex,is_retry) -- or questname
	...
	if quest and questname and quest.name~=questname then -- Whoa, journal changed, title mismatch!
		if is_retry then return nil end  -- failed to retry
		self:UpdateJournal()
...
function Quests:GetCompletionStatus(qname,condtxt)
	...
	-- QUEST: COMPLETE? Perhaps it's all done?
		...
		if isComplete then return true,true,"quest complete" end  -- Whole quest is complete. Cheers.
	-- ... if not, keep checking.

	-- Check quest in journal.
		local quest = self:GetQuest(qname)
		if not quest then return false,false,"not in journal" end

		if quest:HasRecentlyCompletedCondition(condtxt) then return true,true,"cond recently completed" end
	--

	-- STEP: PINPOINT. Use condtxt if need be.
	local stepnum,condnum = quest:FindStepCond(condtxt)

	if not stepnum then return false,false,"no step matched",condtxt end  -- no step? too bad.
	...
	if complete then return complete,possible,"step completion" end
...
local function textmatch(subject,test)
	if not subject or not test then return false end
	if subject==test then return true end
	if zo_plainstrfind(test,"*") and subject:match(test) then return true end
	return false
end
...
function Quest:FindStepCond(condtxt)
	for snum,s in ipairs(self.steps) do
		if textmatch(s.trackerText,condtxt) then return snum,0 end  -- trackerText matched?
		if s.conditions then for cnum,c in ipairs(s.conditions) do
			if textmatch(c.text,condtxt) then return snum,cnum end
		end end
	end
	return nil,nil
end
...
function Quest:HasRecentlyCompletedCondition(condtxt)
	if not self.oldsteps then return false end
	for snum,s in ipairs(self.oldsteps) do
		if textmatch(s.trackerText,condtxt) then return true end  -- trackerText matched?
		if s.conditions then for cnum,c in ipairs(s.conditions) do
			if textmatch(c.text,condtxt) then return true end
		end end
	end
	return false
end
...
--[[
	...
	function Quest:AddNewStepInfo(journalIndex)
		journalIndex = journalIndex or Quests:FindQuest(self.name)
